...
class observer {
    public static function observe_submission(\mod_questionnaire\event\attempt_submitted $event) {
        ...
        try {
            // جلب إعدادات الإضافة (ما كتبه الأدمن)
            $config = get_config('local_varkassign');
            
            // إذا لم يقم الأدمن بضبط الإعدادات، نستخدم القيم الافتراضية
            $group_v_name = !empty($config->group_v) ? trim($config->group_v) : 'Vark_V';
            $group_a_name = !empty($config->group_a) ? trim($config->group_a) : 'Vark_A';
            $group_r_name = !empty($config->group_r) ? trim($config->group_r) : 'Vark_R';
            $group_k_name = !empty($config->group_k) ? trim($config->group_k) : 'Vark_K';

            // ... (بقية كود جلب البيانات والاستبيان كما هو) ...
            $data = $event->get_data();
            ...
            if (empty($rid)) {
                $sql_get_rid = "SELECT MAX(id) FROM {questionnaire_response} 
                                WHERE questionnaireid = :qid AND userid = :uid AND complete = 'y'";
            ...
            // 6. خريطة المجموعات (تستخدم المتغيرات الآن)
            $group_map = [
                'V' => $group_v_name,
                'A' => $group_a_name,
                'R' => $group_r_name,
